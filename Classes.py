'''
МЕСЕНДЖЕР TWO CUPS (далее М).

Концепция:
(концепция является ориентиром и реализация может отличаться)

СЕРВЕР (далее С) делает все.

КЛИЕНТ (далее К) очень легкий, что бы работал даже на ламповом калькуляторе.
...
К заходит в М либо создавая нового Юзера либо представляется как уже
...
начать чатиться с любым из него, либо выбрать Юзера из своего Списка
...
Если Старый то ищет его в таблице Юзеры. Авторизует если ок, иначе говорит К что
такого нет.
...
Собеседника\ов. Пересылает сбщ между собеседниками.
